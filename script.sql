...
select task.id, task.title, task.note, task_board.name, user.username from task left join task_board on task.board_id = task_board.id left join user on user.id = task.user_id;
